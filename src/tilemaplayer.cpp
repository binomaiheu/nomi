...
TileMapLayer::TileMapLayer(const tmx::Map &map, std::size_t idx)
{
...
void TileMapLayer::setTile(int tileX, int tileY, tmx::TileLayer::Tile tile, bool refresh )
{
...
tmx::TileLayer::Tile TileMapLayer::getTile(int tileX, int tileY)
{
    sf::Vector2u chunkLocale;
    const auto &selectedChunk = getChunkAndTransform(tileX, tileY, chunkLocale);
...
void TileMapLayer::setColor(int tileX, int tileY, sf::Color color, bool refresh )
{
    sf::Vector2u chunkLocale;
    const auto &selectedChunk = getChunkAndTransform(tileX, tileY, chunkLocale);
...
sf::Color TileMapLayer::getColor(int tileX, int tileY)
{
    sf::Vector2u chunkLocale;
    const auto &selectedChunk = getChunkAndTransform(tileX, tileY, chunkLocale);
...
TileMapLayer::Chunk::Ptr &TileMapLayer::getChunkAndTransform(int x, int y, sf::Vector2u &chunkRelative)
{
...
void TileMapLayer::createChunks(const tmx::Map &map, const tmx::TileLayer &layer)
{
...
void TileMapLayer::updateVisibility(const sf::View &view) const
{
...
void TileMapLayer::update(sf::Time elapsed)
{
...
// implementatino of the chunk private subclass

TileMapLayer::Chunk::Chunk( const tmx::TileLayer& layer, 
...
void TileMapLayer::Chunk::generateTiles(bool registerAnimation )
{
...
void TileMapLayer::Chunk::setTile(int x, int y, tmx::TileLayer::Tile tile, bool refresh)
{
    m_chunkTileIDs[calcIndexFrom(x, y)] = tile;
    maybeRegenerate(refresh);
}

sf::Color TileMapLayer::Chunk::getColor(int x, int y) const
{
    return m_chunkColors[calcIndexFrom(x, y)];
}

void TileMapLayer::Chunk::setColor(int x, int y, sf::Color color, bool refresh)
{
    m_chunkColors[calcIndexFrom(x, y)] = color;
    maybeRegenerate(refresh);
}

void TileMapLayer::Chunk::maybeRegenerate(bool refresh)
{
    if (refresh)
    {
        for (const auto &ca : m_chunkArrays)
...
void TileMapLayer::Chunk::flipY(sf::Vector2f *v0, sf::Vector2f *v1, sf::Vector2f *v2, sf::Vector2f *v3)
{
    //Flip Y
    sf::Vector2f tmp = *v0;
    v0->y = v2->y;
...
void TileMapLayer::Chunk::flipX(sf::Vector2f *v0, sf::Vector2f *v1, sf::Vector2f *v2, sf::Vector2f *v3)
{
    //Flip X
    sf::Vector2f tmp = *v0;
    v0->x = v1->x;
...
void TileMapLayer::Chunk::flipD(sf::Vector2f *v0, sf::Vector2f *v1, sf::Vector2f *v2, sf::Vector2f *v3)
{
    //Diagonal flip
    sf::Vector2f tmp = *v1;
    v1->x = v3->x;
...
void TileMapLayer::Chunk::doFlips(std::uint8_t bits, sf::Vector2f *v0, sf::Vector2f *v1, sf::Vector2f *v2, sf::Vector2f *v3)
{
    //0000 = no change
...
// private ChunkArray class inside a Chunk
TileMapLayer::Chunk::ChunkArray::ChunkArray(const sf::Texture &t, const tmx::Tileset &ts)
    : m_texture(t)
{
...
void TileMapLayer::Chunk::ChunkArray::addTile(const Chunk::Tile &tile)
{
...
void TileMapLayer::Chunk::ChunkArray::draw(sf::RenderTarget &rt, sf::RenderStates states) const
{
